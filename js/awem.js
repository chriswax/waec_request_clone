...
var awem = function ($) {
    ...
    function serializeFormAsObj(frm) {
        var res = {};
        frm.find(':input[name]:not(.o-itmv)').each(function () {
            var el = $(this);
            var v = el.val();
            if (isNotNull(v) && v !== se) {
    ...
    function gridPageSize(o) {
        ...
        $grid.find('.awe-footer').append('<div class="awe-ajaxradiolist-field o-gpgs awe-field"><input id="' + psi + '" class="awe-val" type="hidden" value="' + o.ps + '" /><div class="awe-display form-control form-control-lgg"></div></div>'); // form-control to conform
    ...
    function gridFilterRow(opt) {
        return function (o) {
            ...
            function load() {
                if (!areScritEq(model, serializeFormAsObj(row))) {

                    var res = row.find(':input').serializeArray();
                    model = serializeFormAsObj(row);

                    var norder = [];
                    loop(order, function (name) {
                    ...
                    res = res.concat(awef.serlArr(order, 'forder'));
            ...
            bind(o, grid, 'aweheaderinit', function () {
                ...
                var gutil = gu({ o: o, model: model, prefix: prefix });
                var parseFormat = gutil.parse;
                var getVal = gutil.getVal;
                ...
                var cx = {
                    id: idFunc, model: model, val: getVal, grido: o
                };
                ...
                loop(o.columns,
                    function (col) {
                        ...
                        function defaultFilter(p, elm) {
                            var data = guc(o, p, frow);
                            if (data) {
                                var id = idFunc(p);
                                ...
                                fchtml += lsted(id, pval, '', p, multi);
                                initFuncs.push(function () { mainf(fopt); });
                            }
                        }
                    });
